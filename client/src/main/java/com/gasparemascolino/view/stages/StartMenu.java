package com.gasparemascolino.view.stages;

import com.gasparemascolino.Client;
import com.gasparemascolino.view.GUI;
    ...
    public static void startMenu(Stage startView) {
        ...
        play.setOnAction(e -> {
            ...
            connect.setOnAction(actionEvent -> {
                ...
                try {
                    condition = Client.NewConnection(textField.getText());
                    ...
                    if (!condition) {
                        ...
                        try {
                            ...
                            if (GUI.userindex == 1)
                                GameSettings.gameSettings(startView);
                            else {
                                //numofplayers update
                                GUI.receiveMessage();
                                System.out.println("Il numero di giocatori è: " + GUI.numofplayers);

                                for (int i = 0; i < GUI.numofplayers; i++)
                                    GUI.divinity.add(new Text("Divinity " + i));

                                JoinLobby.joinLobby(startView, GUI.numofplayers);
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
