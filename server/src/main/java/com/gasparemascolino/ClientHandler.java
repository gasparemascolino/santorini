...
import com.gasparemascolino.controller.divinities.Chronus;
...
import static com.gasparemascolino.NotifyServer.*;
import static com.gasparemascolino.Server.*;
import static com.gasparemascolino.Server.board;
...
public class ClientHandler implements Runnable {
    ...
    public static Cell cellToMoveOn(Worker worker, GameBoard board) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        while (!condition) {
            ...
            condition = WorkerController.Move(worker, row, column, board);
    ...
    public static Cell cellToBuildOn(Worker worker, GameBoard board) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        Cell[] availableposition = WorkerController.enableToBuild(board.players.get(currentclient-1).getWorker(worker.getIndex()), board);
        ...
        while (!condition) {
            ...
            condition = WorkerController.Build(worker, row, column, board);
    ...
    public static void looseCondition(int workerindex1, int workerindex2) throws IOException, InterruptedException {
        ...
        Cell[] availablepostion1 = WorkerController.enableToMove(board.players.get(currentclient-1).getWorker(workerindex1), board);
        Cell[] availableposition2 = WorkerController.enableToMove(board.players.get(currentclient-1).getWorker(workerindex2), board);

        if(Chronus.getChronusPower()){
            if(WorkerController.completetowers>=5){
                for(int i=0;i<playernum;i++){
